...
export default {
  ...
  methods: {
    ...
    exportPDF() {
      ...
      doc.setFontSize(16).text("Creditos Alumnos información", 2.5, 0.7); 
      doc.setLineWidth(0.01).line(0.5, 1.1, 8.0, 1.1)
      doc.autoTable({
          headStyles: { fillColor: [199, 0, 57]}, 
          body: this.eventos, 
          columns : [
            {header: "Titulo de evento", dataKey: "tituloEvento"},
            {header: "Unidad responsable", dataKey: "unidadResponsable"},
            {header: "Fecha de evento", sortable: true, dataKey: "fechaEvento"},
